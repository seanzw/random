...
def parse_benchmark_file(filename):
    ...
    for line in lines:
        ...
        # Check for "Out of SHMEM" cases (handle both old format and new "Passed |" format)
        out_of_shmem_match = re.search(
            r"(?:Passed\s*\|\s*)?Stages=\s*(\d+).*Out of SHMEM", line
        )
    ...
    for benchmark in benchmark_data:
        ...
        for i, chunk in enumerate(chunk_sizes):
            for j, stage in enumerate(stages_list):
                if (
                    chunk in benchmark_data[benchmark]
                    and stage in benchmark_data[benchmark][chunk]
                ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot TMA and cp.async benchmark results"
    )
    ...
    available_configs = [
        (key, title) for key, title in subplot_config if key in benchmark_data
    ]
    ...
    for row, (benchmark_key, title) in enumerate(available_configs):
        if benchmark_key in benchmark_data:
            ...
            util_matrix = (
                bw_matrix / 1000 / peak_bw_tb * 100
            )  # Convert GB/s to TB/s, then to percentage
            ...
            ax_bw.set_title(
                f"{title} - Bandwidth (GB/s)", fontsize=16, fontweight="bold"
            )
